...
def add_arguments(argument_parser: ArgumentParser, name: str, class_ref: Type[T]) -> T:
    ...
    argument_values = {
        argument.name: vars(temp_args)[argument.name] for argument in arguments
    }
...
def add_options_from_module(
    argument_parser: ArgumentParser, name: str, module: ModuleType, of_subclass: Type[T]
) -> T:
    def predicate(obj):
